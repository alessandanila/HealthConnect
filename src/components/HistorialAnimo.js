...
const HistorialAnimo = () => {
    const [selectedTab, setSelectedTab] = useState('Gráfico de barras');
    ...
    // Función para obtener el historial de ánimo (simulada con datos estáticos)
    const obtenerHistorialAnimoSimulado = () => {
        return [
            { log_id: 1, log_date: '2025-05-01', mood_score: 4, notes: 'Me sentí bastante bien.' },
            { log_id: 2, log_date: '2025-05-05', mood_score: 5, notes: 'Un día excelente.' },
            { log_id: 3, log_date: '2025-05-10', mood_score: 3, notes: 'Día normal.' },
            { log_id: 4, log_date: '2025-05-15', mood_score: 2, notes: 'Me sentí un poco bajo de ánimo.' },
            ...
            { log_id: 7, log_date: '2025-05-30', mood_score: 5, notes: '¡Genial!' },
            ...
            { log_id: 11, log_date: '2025-06-19', mood_score: 2, notes: 'Día difícil.' },
    ...
    useEffect(() => {
        ...
        setMoodLogs(data);
        // Procesar datos iniciales para el gráfico de barras (últimos 30 días)
        const last30DaysLogs = data.slice(-12); // Tomando los últimos 12 para simular 30 días
        ...
        // Preparar datos para el gráfico de líneas también
    ...
    return (
        <div className={styles.container}>
            <div className={styles.header}>
                ...
                <h2 className={styles.title}>Historial del estado de ánimo</h2>
            ...
            <div className={styles.filter}>
                ...
                <div className={styles.datePickerContainer}>
                    ...
                    <DatePicker
                        ...
                        placeholderText="Seleccione la fecha de finalización"
                    />
                </div>
            ...
            <div className={styles.tabs}>
                <button
                    className={`${styles.tab} ${selectedTab === 'Gráfico de barras' ? styles.activeTab : ''}`}
                    onClick={() => handleTabChange('Gráfico de barras')}
                >
                    Gráfico de barras
                </button>
                <button
                    className={`${styles.tab} ${selectedTab === 'Gráfico de líneas' ? styles.activeTab : ''}`}
                    onClick={() => handleTabChange('Gráfico de líneas')}
                >
                    Gráfico de líneas
                </button>
                <button
            ...
            <div className={styles.content}>
                {selectedTab === 'Gráfico de barras' && chartData.labels.length > 0 && (
                    <div className={styles.chartContainer}>
                        <h3 className={styles.chartTitle}>Tendencias del estado de ánimo - Últimos 30 días</h3>
                        <Bar data={chartData} options={chartOptions} />
                    </div>
                )}
                {selectedTab === 'Gráfico de líneas' && lineChartData && lineChartData.labels.length > 0 && (
                    <div className={styles.chartContainer}>
                        <h3 className={styles.chartTitle}>Tendencias del estado de ánimo</h3>
                        <Line data={lineChartData} options={chartOptions} />
                    </div>
                )}
                {selectedTab === 'Vista de calendario' && (
                    <div className={styles.calendarContainer}>
                        <h3>Vista de calendario (Próximamente)</h3>
                        {/* Aquí iría la lógica para mostrar un calendario */}
                        <p>Esta función estará disponible pronto.</p>
                    </div>
                )}
                {moodLogs.length === 0 && selectedTab !== 'Vista de calendario' && (
                    <p className={styles.noData}>No hay registros de estado de ánimo disponibles.</p>
                )}
            </div>

            <div className={styles.detailsContainer}>
                <h3 className={styles.detailsTitle}>Detalles del estado de ánimo</h3>
                <ul className={styles.detailsList}>
                    {moodLogs.map((log) => (
                        <li key={log.log_id} className={styles.detailItem}>
                            <div className={styles.moodIcon}>
                                <div className={styles.iconPlaceholder}>😊</div> {/* Icono basado en el estado de ánimo */}
                            </div>
                ...
                {moodLogs.length === 0 && <p className={styles.noData}>No hay detalles de estado de ánimo para mostrar.</p>}
            </div>
